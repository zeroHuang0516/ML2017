...
# add unlabel feature to training data    
def add_unlabel_featrue_to_training(training_feature, training_yhat, unlabel_feature, model, confidence_th):
...
dev_feats, dev_labels,_ = read_dataset(sys.argv[2])
# read testing raw data
test_feats= read_test_dataset(sys.argv[3])
# get training_feature , training_yhat , validation_feature , validation_yhat
...
for i in range(nb_add_unlabel):	
    if i == nb_add_unlabel -1:
        ...
        emotion_classifier.fit(training_feature, training_yhat, batch_size=128, nb_epoch=40,validation_data=(validation_feature, validation_yhat), callbacks=[early_stop])
    else:
        emotion_classifier = build_model('easy')
        emotion_classifier.fit(training_feature, training_yhat, batch_size=128, nb_epoch =40, validation_data=(validation_feature, validation_yhat), callbacks=[early_stop] )
        (training_feature, training_yhat, unlabel_feature) = add_unlabel_featrue_to_training(training_feature, training_yhat, unlabel_feature, emotion_classifier, confidence_th)
emotion_classifier = build_model('easy')
  
# save model    
emotion_classifier.save('semi_model')
